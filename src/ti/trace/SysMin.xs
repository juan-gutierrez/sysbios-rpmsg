...
function module$static$init(obj, params)
{
	...
	print("SYSMIN", segment, segment.base);
    ...
    if (Program.platformName.match(/ipu/)) {
        obj.lineBuffers.length = BIOS.numCores;

        for (var i = 0; i < BIOS.numCores; i++) {
            obj.lineBuffers[i].outidx = 0;
            for (var j = 0; j < 256; j++) {
                obj.lineBuffers[i].outbuf[j] = 0;
            }
        }
    }
    else {
        obj.lineBuffers.length = 0;
        //obj.lineBuffers[0] = null;
    }
